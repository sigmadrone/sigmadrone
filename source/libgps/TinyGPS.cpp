...
void TinyGPS::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
  ...
  if (fix_age) *fix_age = _last_position_fix == (unsigned long)GPS_INVALID_FIX_TIME ?
   (unsigned long)GPS_INVALID_AGE : millis() - _last_position_fix;
}
...
void TinyGPS::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  if (date) *date = _date;
  if (time) *time = _time;
  if (age) *age = _last_time_fix == (unsigned long)GPS_INVALID_FIX_TIME ?
   (unsigned long)GPS_INVALID_AGE : millis() - _last_time_fix;
}
