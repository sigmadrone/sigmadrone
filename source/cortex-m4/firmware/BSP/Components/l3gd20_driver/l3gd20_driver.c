...
* Return			: None
*******************************************************************************/
u8_t L3GD20_ReadReg8(u8_t Reg, u8_t* Data) {
  
  //To be completed with either I2c or SPI reading function
  //i.e.: *Data = SPI_Mems_Read_Reg( Reg );
  GYRO_IO_Read(Data, Reg, 1);
  return 1;
}
...
* Return			: None
*******************************************************************************/
u8_t L3GD20_WriteReg8(u8_t Reg, u8_t Data) {
    
  //To be completed with either I2c or SPI writing function
  //i.e.: SPI_Mems_Write_Reg(Reg, Data);
  GYRO_IO_Write(&Data, Reg, 1);
  return 1;
}
/* Private functions ---------------------------------------------------------*/
...
status_t L3GD20_SetODR(L3GD20_ODR_t ov){
  ...
  if( !L3GD20_ReadReg8(L3GD20_CTRL_REG1, &value) )
    return MEMS_ERROR;

  value &= 0x0f;
  value |= ov<<4;

  if( !L3GD20_WriteReg8(L3GD20_CTRL_REG1, value) )
    return MEMS_ERROR;
...
status_t L3GD20_SetFullScale(L3GD20_Fullscale_t fs) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_CTRL_REG4, &value) )
    return MEMS_ERROR;
                  
  value &= 0xCF;	
  value |= (fs<<L3GD20_FS);
  
  if( !L3GD20_WriteReg8(L3GD20_CTRL_REG4, value) )
    return MEMS_ERROR;
...
status_t L3GD20_HPFEnable(State_t hpf) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_CTRL_REG5, &value) )
    return MEMS_ERROR;
                  
  value &= 0xEF;
  value |= (hpf<<L3GD20_HPEN);
  
  if( !L3GD20_WriteReg8(L3GD20_CTRL_REG5, value) )
    return MEMS_ERROR;
...
status_t L3GD20_SetHPFMode(L3GD20_HPFMode_t hpf) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_CTRL_REG2, &value) )
    return MEMS_ERROR;
                  
  value &= 0xCF;
  value |= (hpf<<L3GD20_HPM);
  
  if( !L3GD20_WriteReg8(L3GD20_CTRL_REG2, value) )
    return MEMS_ERROR;
...
status_t L3GD20_SetIntPinMode(L3GD20_IntPinMode_t pm) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_CTRL_REG3, &value) )
    return MEMS_ERROR;
                  
  value &= 0xEF;
  value |= (pm<<L3GD20_PP_OD);
  
  if( !L3GD20_WriteReg8(L3GD20_CTRL_REG3, value) )
    return MEMS_ERROR;
...
status_t L3GD20_Int1LatchEnable(State_t latch) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_INT1_CFG, &value) )
    return MEMS_ERROR;
                  
  value &= 0xBF;
  value |= latch<<L3GD20_LIR;
  
  if( !L3GD20_WriteReg8(L3GD20_INT1_CFG, value) )
    return MEMS_ERROR;
...
status_t L3GD20_ResetInt1Latch(void) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_INT1_SRC, &value) )
    return MEMS_ERROR;
...
status_t L3GD20_SetInt1Threshold(L3GD20_IntThsAxis axis, u16_t ths) {
  ...
  switch (axis) {
    ...
    case L3GD20_THS_X:
      ...
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_XL, value) )
        return MEMS_ERROR;
      
      //write the threshold LSB
      value = (u8_t)( ths >> 8); 
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_XH, value) )
        return MEMS_ERROR;
    ...
    case L3GD20_THS_Y:
      //write the threshold LSB
      value = (u8_t)( ths & 0x00ff); 
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_YL, value) )
        return MEMS_ERROR;
      
      //write the threshold LSB
      value = (u8_t)( ths >> 8); 
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_YH, value) )
        return MEMS_ERROR;
    ...
    case L3GD20_THS_Z:
      //write the threshold LSB
      value = (u8_t)( ths & 0x00ff); 
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_ZL, value) )
        return MEMS_ERROR;
      
      //write the threshold LSB
      value = (u8_t)( ths >> 8); 
      if( !L3GD20_WriteReg8(L3GD20_INT1_THS_ZH, value) )
        return MEMS_ERROR;
...
status_t L3GD20_SetInt1Duration(L3GD20_Int1Conf_t id) {
  ...
  if( !L3GD20_WriteReg8(L3GD20_INT1_DURATION, id) )
    return MEMS_ERROR;
...
status_t L3GD20_FIFOModeEnable(L3GD20_FifoMode_t fm) {
  ...
  if(fm == L3GD20_FIFO_DISABLE) {
    ...
    if( !L3GD20_ReadReg8(L3GD20_FIFO_CTRL_REG, &value) )
      return MEMS_ERROR;
    
    value &= 0x1f;
    value |= (fm<<L3GD20_FM0);
    
    if( !L3GD20_WriteReg8(L3GD20_FIFO_CTRL_REG, value) )
      return MEMS_ERROR;
  }
...
status_t L3GD20_GetSatusReg(u8_t* buff) {
  if( !L3GD20_ReadReg8(L3GD20_STATUS_REG, buff) )
      return MEMS_ERROR;
...
status_t L3GD20_GetAngRateRaw(AxesRaw_t* buff) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_OUT_X_L, &valueL) )
      return MEMS_ERROR;
  
  if( !L3GD20_ReadReg8(L3GD20_OUT_X_H, &valueH) )
      return MEMS_ERROR;
  
  buff->AXIS_X = (i16_t)( (valueH << 8) | valueL );
  
  if( !L3GD20_ReadReg8(L3GD20_OUT_Y_L, &valueL) )
      return MEMS_ERROR;
  
  if( !L3GD20_ReadReg8(L3GD20_OUT_Y_H, &valueH) )
      return MEMS_ERROR;
  
  buff->AXIS_Y = (i16_t)( (valueH << 8) | valueL );
  
   if( !L3GD20_ReadReg8(L3GD20_OUT_Z_L, &valueL) )
      return MEMS_ERROR;
  
  if( !L3GD20_ReadReg8(L3GD20_OUT_Z_H, &valueH) )
      return MEMS_ERROR;
...
status_t L3GD20_GetInt1Src(u8_t* buff) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_INT1_SRC, buff) )
    return MEMS_ERROR;
...
status_t L3GD20_GetFifoSourceReg(u8_t* buff) {
  ...
  if( !L3GD20_ReadReg8(L3GD20_FIFO_SRC_REG, buff) )
    return MEMS_ERROR;
