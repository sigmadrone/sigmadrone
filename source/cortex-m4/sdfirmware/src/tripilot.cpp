...
Vector3f TriPilot::get_torque(const DroneState& state)
{
	...
	torq_xy_i_ = pid_.get_i(error_xy, state.dt_.seconds_float(), state.leakrate_);
	torq_xy_p_ = pid_.get_p(error_xy);
	torq_xy_d_ = pid_.get_d(error_xy, state.dt_.seconds_float());
	torq_z_p_ = pidz_.get_p(error_z);
	torq_z_d_ = pidz_.get_d_median(error_z, state.dt_.seconds_float());
	...
	if (target_thrust_ < 0.1)
		torq *= std::pow(target_thrust_ / 0.1, 2.0);
