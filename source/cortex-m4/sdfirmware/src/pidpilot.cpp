...
void PidPilot::update_state(DroneState& state, const QuaternionF& target_attitude)
{
	...
	//  0.6 --> 0.10125 kg.m
	torque_rpm = torque_correction_ * 0.6 / (101.25/1000.0) / 2.0;

	motors_ = Vector4f(
			target_thrust_ + Vector3d::dot(torque_rpm, m0_),
			target_thrust_ + Vector3d::dot(torque_rpm, m1_),
			target_thrust_ + Vector3d::dot(torque_rpm, m2_),
			target_thrust_ + Vector3d::dot(torque_rpm, m3_));
...
const Vector4f& PidPilot::get_motors() const
{
	return motors_;
}
