...
//			trace_printf("recved: %s\n", disp);
			trace_printf("MSP: 0x%X, PSP: 0x%X\n", __get_MSP(), __get_PSP());
		}
...
namespace
{
  ...
  constexpr uint32_t BLINK_ON_TICKS = configTICK_RATE_HZ * 2 / 3;
  constexpr uint32_t BLINK_OFF_TICKS = configTICK_RATE_HZ - BLINK_ON_TICKS;
}
...
{
	DigitalOut green(USER_LED1), red(USER_LED2);
	uint32_t seconds = 0;
...
void secondary_task(void *params)
{
	uint32_t seconds = 0;
	// Infinite loop
	while (1) {
...
{
	/*
	 * Relocate the system interrupt table to RAM and modify it.
	 */
	relocate_interrupt_table();
